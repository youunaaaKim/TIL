def comb(arr, r):
    ...
    for i in range(len(arr)): # 첫 번째로 누구 뽑을 지의 모든 경우의 수. -> i가 0일 때는 1번 선수, i 가 1일 때는 2번 선수
        # i번째 원소를 첫 번째 요소로 선택
        elem = arr[i]

        # i번째 원소 이후의 나머지 리스트에서
        # 나머지 (r-1)개의 조합을 재귀적으로 구함
        for rest in comb(arr[i + 1 :], r - 1): # -> ([2, 3, 4], 2)
            # 나머지 3명 중 2명을 뽑는 모든 조합 구하기 그리고 위에 elem에 붙기
